...
// 確認參數是否正確
if (!isset($pid)) die ("Parameter error!");
// 找出此用戶的群組
...
if (count($rs) <= 0) die("找不到編號為 $pid 之資料");
$GID = $rs[0]['owner_id'];
if ($GID<>$UserOwnerID) {   // 非本單位人員，看看是否有額外權限
    $sqlcmd = "SELECT privilege FROM privileges "
        . "WHERE loginid='$LoginID' and uid='$GID' AND privilege>1";
    $rs = querydb($sqlcmd, $db_conn);
    if (count($rs) <= 0) die("您對編號 $pid 之資料無修改權限");
}
// =====================================================
...
if (isset($Confirm)) {   // 確認按鈕
    ...
    // Demo for the reason to use addslashes
        ...
        $sqlcmd="UPDATE product SET pname='$Name',price='$Price',pnum='$Pnum',deadline='$Deadline',class_no='$Class_no',owner_id='$UserOwnerID' "
            . " WHERE pid='$pid'";
