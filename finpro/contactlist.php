...
$sqlcmd = "SELECT * FROM user_table WHERE uid='$owner_id' ";
$rs = querydb($sqlcmd, $db_conn);
...
if (isset($action) && $action=='recover' && isset($pid)) {
    // Recover this item
    $sqlcmd = "SELECT * FROM product WHERE pid='$pid' AND pnum='0'";
    $rs = querydb($sqlcmd, $db_conn);
    if (count($rs) > 0) {
        $sqlcmd = "UPDATE product SET pnum='1' WHERE pid='$pid'";
...
if (isset($action) && $action=='delete' && isset($pid)) {
	...
	//echo $pid;
    if (count($rs) > 0) {
        $sqlcmd = "UPDATE product SET pnum=0 WHERE pid='$pid' ";
        $result = updatedb($sqlcmd, $db_conn);
    }
}
...
$sqlcmd = "SELECT count(*) AS reccount FROM product WHERE product.pnum>0 AND owner_id IN $OwnerIDs";
$rs = querydb($sqlcmd, $db_conn);
...
$sqlcmd = "SELECT * FROM product,product_class WHERE product.pnum>0 AND product.class_no=product_class.class_id AND owner_id IN $OwnerIDs"
...
/*
$sqlcmd = "SELECT * FROM product,user_table,product_class WHERE product.owner_id=user_table.uid AND product.class_no=product_class.class_id";
$Contacts = querydb($sqlcmd, $db_conn);
*/
  ...
  <td align="right" width="30%">
	...
	<div style="text-align:center;">
		...
		<table border="0" width="90%" align="center">
			...
			<a style="color:#FF0000" href="contactlist.php">我的賣場</a><br>
			...
			foreach ($Contacts AS $item) {
			  ...
			  $UName = 'N/A';
			  if (isset($arrGroups["$owner_id"])) $UName = $arrGroups["$owner_id"];
			  $DspMsg = "'確定刪除項目?'";
			  $PassArg = "'contactlist.php?action=delete&pid=$pid'";
			...
			<?php
			  if ($pnum=='0') {
			?>
			  <a href="contactlist.php?action=recover&pid=<?php echo $pid; ?>">
				回復
				</a></td>
			...
			<?php } else { ?>
			  ...
			  修改</a>&nbsp;
			  <a href="upload.php?pid=<?php echo $pid; ?>">
			  照片</a>
			  </td>
